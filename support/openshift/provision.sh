...
function usage() {
    ...
    echo
    echo "Example:"
    echo " $0 setup rhdm7-cicd --project-suffix s40d"
    echo
    echo "COMMANDS:"
    ...
    echo
    echo "DEMOS:"
    echo "   rhdm7-cicd               Red Hat Decision Manager CI/CD demo"
    echo
    echo "OPTIONS:"
...
PRJ=("rhdm7-cicd-$PRJ_SUFFIX" "RHDM7 CI/CD Demo" "Red Hat Decision Manager 7 CI/CD Demo")
...
function create_projects() {
  ...
  oc new-project "${PRJ[0]}" --display-name="${PRJ[1]} Development" --description="${PRJ[2]} Development" >/dev/null
  oc new-project "${PRJ[0]}-qa" --display-name="${PRJ[1]} QA" --description="${PRJ[2]} QA" >/dev/null
  oc new-project "${PRJ[0]}-prod" --display-name="${PRJ[1]} Production" --description="${PRJ[2]} Production" >/dev/null
...
function deploy_jenkins() {
  ...
  #TODO: Should we allow non-persistent deployments? Decision Central uses PVs OOTB as well ... So we might want to just make everything persistent.

  ARG_EPHEMERAL=false
  if [ "$ARG_EPHEMERAL" = true ] ; then
    oc new-app jenkins-ephemeral -l app=jenkins -p MEMORY_LIMIT=1Gi -n $PRJ_CI
  else
    #TODO: Set the archict
...
function deploy_jenkins_maven_slave() {
  ...
  oc process -f jenkins-maven-slave-template.yaml -p DOCKERFILE_REPOSITORY="http://www.github.com/jboss-" -p DOCKERFILE_REF="openshift-build" -p DOCKERFILE_CONTEXT=support/openshift/jenkins-maven-slave -n $PRJ_CI | oc create -f - -n $PRJ_CI
}
